...
interface AuthorReposDrawerProps {
  open: boolean;
  owner: string | null;
  repos: Repo[];
  loading: boolean;
...
const OwnerReposDrawer = ({ open, owner, repos, loading, error, hasMore, onClose, onLoadMore } : AuthorReposDrawerProps) => {
  return (
    <Drawer
        anchor="right"
        open={open}
        ...
        >
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <Typography variant="h6">
                    {owner ? `Repos by ${owner}` : "Author Repos"}
                </Typography>
